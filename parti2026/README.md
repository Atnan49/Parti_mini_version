...
- ☁️ **Panduan Deployment Produksi**:
  - **Render.com (Docker Container)**: Menggunakan `Dockerfile` bawaan, sesuaikan `APP_KEY`, `DB_CONNECTION=pgsql`, dan jalankan `/run-migration` serta `/run-seed`.
  - **Hostinger / Shared Hosting**: Pastikan Git terhubung ke branch `atnan-dev`, ubah root direktori ke `public`, dan jalankan `/create-symlink` untuk storage.
