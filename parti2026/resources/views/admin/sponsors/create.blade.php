...
<div class="max-w-2xl mx-auto space-y-6 text-left">
    ...
    <!-- Header -->
    <div>
        <h1 class="font-display font-bold text-2xl text-ink uppercase tracking-wide">Tambah Sponsor Baru</h1>
        <p class="text-ink-soft text-sm mt-1">Daftarkan logo perusahaan sponsor yang mendukung kegiatan event PARTI.</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white border border-line rounded-[6px] p-6 md:p-8 shadow-sm">
        <form method="POST" action="{{ route('admin.sponsors.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name') }}" required autofocus class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" placeholder="Contoh: PT. Sumber Makmur" />
            ...
            <div>
                ...
                <select id="tier" name="tier" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember cursor-pointer">
                    <option value="PLATINUM" @selected(old('tier') === 'PLATINUM')>PLATINUM</option>
                    <option value="GOLD" @selected(old('tier') === 'GOLD')>GOLD</option>
                    <option value="SILVER" @selected(old('tier', 'SILVER') === 'SILVER')>SILVER</option>
                    <option value="BRONZE" @selected(old('tier') === 'BRONZE')>BRONZE</option>
                </select>
            ...
            <!-- Logo File -->
            <div>
                <label for="logo" class="block font-mono text-[11px] tracking-wider uppercase text-ink-soft mb-1.5 font-bold">File Logo Sponsor</label>
                <input id="logo" name="logo" type="file" required class="block w-full text-xs text-ink-soft file:mr-3 file:py-1.5 file:px-3 file:rounded-[2px] file:border file:border-line file:text-xs file:font-semibold file:bg-paper-warm/40 file:text-ink-soft hover:file:bg-paper-warm cursor-pointer" />
            ...
            <div>
                ...
                <input id="website_url" name="website_url" type="url" value="{{ old('website_url') }}" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" placeholder="https://example.com" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="flex items-center pt-6">
                    <label for="is_active" class="inline-flex items-center text-sm text-ink cursor-pointer">
                        <input id="is_active" type="checkbox" name="is_active" value="1" checked class="rounded border-line text-ember focus:ring-ember">
