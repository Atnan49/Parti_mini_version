...
<div class="max-w-2xl mx-auto space-y-6 text-left">
    ...
    <!-- Header -->
    <div>
        <h1 class="font-display font-bold text-2xl text-ink uppercase tracking-wide">Tambah Agenda Timeline</h1>
        <p class="text-ink-soft text-sm mt-1">Tambahkan agenda atau tonggak sejarah (milestone) baru pada alur waktu event PARTI.</p>
    </div>
    ...
    <div class="bg-white border border-line rounded-[6px] p-6 md:p-8 shadow-sm">
        <form method="POST" action="{{ route('admin.timeline.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <input id="title" name="title" type="text" value="{{ old('title') }}" required autofocus class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" placeholder="Contoh: Pembukaan Webinar Nasional" />
            ...
            <div>
                ...
                <select id="sub_event_id" name="sub_event_id" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember cursor-pointer">
                    ...
                    @foreach($subEvents as $sub)
                        <option value="{{ $sub->id }}" @selected(old('sub_event_id') == $sub->id)>{{ $sub->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input id="date" name="date" type="date" value="{{ old('date') }}" required class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" />
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors">{{ old('description') }}</textarea>
