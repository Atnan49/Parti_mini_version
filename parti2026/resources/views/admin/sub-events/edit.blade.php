...
@section('title', 'Edit Sub Acara | PARTI Admin')
...
<div class="max-w-3xl mx-auto space-y-6 text-left">
    ...
    <!-- Header -->
    <div>
        <h1 class="font-display font-bold text-2xl text-ink uppercase tracking-wide">Edit Sub Acara</h1>
        <p class="text-ink-soft text-sm mt-1">Perbarui informasi sub-acara "{{ $subEvent->name }}" dalam rangkaian event PARTI tahun ini.</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.sub-events.update', $subEvent->id) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $subEvent->name) }}" required autofocus class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" />
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="tagline" name="tagline" type="text" value="{{ old('tagline', $subEvent->tagline) }}" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" placeholder="Contoh: Digital Renaissance Era" />
                ...
                <div>
                    ...
                    <select id="type" name="type" required class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember cursor-pointer">
                        <option value="OFFLINE" @selected(old('type', $subEvent->type) === 'OFFLINE')>OFFLINE (Tatap Muka)</option>
                        <option value="ONLINE" @selected(old('type', $subEvent->type) === 'ONLINE')>ONLINE (Daring)</option>
                        <option value="HYBRID" @selected(old('type', $subEvent->type) === 'HYBRID')>HYBRID (Kombinasi)</option>
                    </select>
                ...
                <div>
                    ...
                    <input id="location" name="location" type="text" value="{{ old('location', $subEvent->location) }}" placeholder="Contoh: Gedung J UMS atau Zoom Meeting" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" />
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="pj_names" name="pj_names" type="text" value="{{ old('pj_names', $pjNamesString) }}" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" placeholder="Pisahkan dengan koma, contoh: Rizqi, Tasya" />
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="5" class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors">{{ old('description', $subEvent->description) }}</textarea>
                ...
                <div>
                    ...
                    <input id="order" name="order" type="number" value="{{ old('order', $subEvent->order) }}" min="0" required class="block w-full border border-line rounded-[2px] px-3.5 py-2.5 text-sm bg-paper-warm/20 focus:outline-none focus:border-ember focus:ring-1 focus:ring-ember transition-colors" />
