...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Background transparansi untuk canvas atlas
      ...
      this.items.map(
        item =>
          new Promise(resolve => {
            ...
            img.onerror = () => {
              // Jika load gagal, gunakan canvas kosong dengan teks sebagai placeholder
              const placeholderCanvas = document.createElement('canvas');
              placeholderCanvas.width = 512;
              placeholderCanvas.height = 512;
              const pCtx = placeholderCanvas.getContext('2d');
              pCtx.fillStyle = '#E2650B';
              pCtx.fillRect(0, 0, 512, 512);
              pCtx.fillStyle = '#FFFFFF';
              pCtx.font = 'bold 40px sans-serif';
              pCtx.textAlign = 'center';
              pCtx.textBaseline = 'middle';
              pCtx.fillText(item.title || 'PARTI', 256, 256);
              const fallbackImg = new Image();
              fallbackImg.src = placeholderCanvas.toDataURL();
...
// Expose fungsi inisialisasi ke window untuk dijalankan Alpine.js
window.initInfiniteMenu = (canvas, items, onActiveItemChange, onMovementChange, scale = 1.0) => {
